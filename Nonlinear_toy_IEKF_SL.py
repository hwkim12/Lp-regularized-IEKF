# IEKF-SL+IAS for the Nonlinear Toy Example 
import numpy as np
import pandas as pd
...
# Initial IEKF-SL
IEKF_SL_ens = IEKF_SL(u0, alpha, inner_niter, h_u, y, P2, R, num_ens)
IEKF_SL_ens = pd.DataFrame(IEKF_SL_ens)
U2 = IEKF_SL_ens.apply(np.mean, axis = 0, result_type ='expand') 
U2 = np.array(U2)
THETA_2 = np.array((U2**2)**(1/(r+1))/(2*r)**(1/(r+1))) ### Lp regularization ###
P2 = np.diag(THETA_2)

# Constructing Approximate Credible Intervals
IEKF_SL_ens_mat = np.matrix(IEKF_SL_ens)
tsin_cos_mat_temp = np.matrix(tsin_cos_mat)
IEKF_SL_recov = IEKF_SL_ens_mat@tsin_cos_mat_temp
IEKF_SL_recov = pd.DataFrame(IEKF_SL_recov)

lower_curve_IEKF_SL = IEKF_SL_recov.apply(np.quantile, axis = 0, q = 0.025, result_type ='expand')
lower_curve_IEKF_SL = np.array(lower_curve_IEKF_SL)
upper_curve_IEKF_SL = IEKF_SL_recov.apply(np.quantile, axis = 0, q = 0.975, result_type ='expand') 
upper_curve_IEKF_SL = np.array(upper_curve_IEKF_SL)
...
ax[0].plot(xgrid, sin_cos_mat@U2, label = 'IEKF-SL', color = 'blue', alpha = 1)
ax[0].fill_between(xgrid, lower_curve_IEKF_SL, upper_curve_IEKF_SL, alpha = 0.2, color = 'blue')
#ax[0].set_title('Recovery based on 1st L0.5-IEKF-SL', fontsize = 16)
ax[0].set_title('Recovery based on 1st L1-IEKF-SL', fontsize = 16)
ax[0].tick_params(axis='both', which='major', labelsize=12)
ax[0].legend(loc = "upper right", fontsize = 12)
ax[0].set_ylim(-4,5)
avg_width.append(np.mean(upper_curve_IEKF_SL-lower_curve_IEKF_SL))
l2_error.append(np.linalg.norm(true_u-U2)/np.sqrt(len(true_u)))

# Outer iteration for Lp-IEKF-SL
for k in range(3):
    IEKF_SL_ens = IEKF_SL(u0, alpha, inner_niter, h_u, y, P2, R, num_ens)
    IEKF_SL_ens = pd.DataFrame(IEKF_SL_ens)
    U2 = IEKF_SL_ens.apply(np.mean, axis = 0, result_type ='expand') 
    ...
    if k == 0:
        ...
        ax[1].plot(xgrid, sin_cos_mat@U2, label = 'IEKF-SL', color = 'blue', alpha = 1)
        ax[1].fill_between(xgrid, lower_curve_IEKF_SL, upper_curve_IEKF_SL, alpha = 0.2, color = 'blue')
        #ax[1].set_title('Recovery based on 2nd L0.5-IEKF-SL', fontsize = 16)
        ax[1].set_title('Recovery based on 2nd L1-IEKF-SL', fontsize = 16)
    ...
    if k == 2:
        ...
        ax[2].plot(xgrid, sin_cos_mat@U2, label = 'IEKF-SL', color = 'blue', alpha = 1)
        ax[2].fill_between(xgrid, lower_curve_IEKF_SL, upper_curve_IEKF_SL, alpha = 0.2, color = 'blue')
        #ax[2].set_title('Recovery based on 4th L0.5-IEKF-SL', fontsize = 16)
        ax[2].set_title('Recovery based on 4th L1-IEKF-SL', fontsize = 16)
