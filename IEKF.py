...
def IEKF(x0, alpha, inner_niter, h_x, y, P, R, num_ens):
    ## x0: Prior Mean
    ## alpha: Step Size
    ...
    for i in range(inner_niter):
        ...
        K_i = Puu_0@np.transpose(H_i)@np.linalg.inv(H_i@Puu_0@np.transpose(H_i)+R) ## kalman gain
        y_i = np.random.multivariate_normal(mean = y, cov = R/alpha, size = num_ens)

        for k in range(num_ens):
            U.iloc[k,:] = U.iloc[k,:] + alpha*((K_i@(y_i[k,:]-hu.iloc[k,:])) + (np.diag(np.ones(p)) - K_i@H_i)@(U0.iloc[k,:] - U.iloc[k,:]))
...
#print(IEKF(x0, 0.5, 20, h_x, y, P, R, num_ens))