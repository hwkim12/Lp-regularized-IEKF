### IEKF-SL+IAS for the Darcy-Flow Example 
import numpy as np
import pandas as pd
...
for k in range(7):       
    x_mat = IEKF_SL(x0 = np.zeros(len(u)), alpha = alpha, inner_niter = 30, h_x = h_u, y = y, P = P_x, R = R, num_ens = num_ens)
    ...
    l2_error.append(np.linalg.norm(x-u)/np.sqrt(len(x)))


    if k == 0:
        ax[0].plot(range(len(u)), u, 'D', label = 'Truth', color = 'red', alpha = 1)
        ax[0].plot(range(len(u)), x, 'o', label = 'IEKF-SL', color = 'blue', alpha = 1)
        ax[0].errorbar(range(len(u)), x, yerr = (x-x_l, x_u-x), fmt='.', color='blue', ecolor='skyblue', capsize=5, capthick = 1.25, label='IEKF-SL-CR')
        #ax[0].set_title('Recovery based on 1st L05-IEKF-SL', fontsize = 16)
    ...
    if k == 3:
        ...
        ax[1].plot(range(len(u)), x, 'o', label = 'IEKF-SL', color = 'blue', alpha = 1)
        ax[1].errorbar(range(len(u)), x, yerr = (x-x_l, x_u-x), fmt='.', color='blue', ecolor='skyblue', capsize=5, capthick = 1.25, label='IEKF-SL-CR')
        #ax[1].set_title('Recovery based on 4th L05-IEKF-SL', fontsize = 16)
        ax[1].set_title('Recovery based on 4th L1-IEKF-SL', fontsize = 16)
        ax[1].tick_params(axis='both', which='major', labelsize=12)
        ax[1].legend(loc = "upper right", fontsize = 12)
        ax[1].set_ylim(-1,1)
    if k == 6:
        ax[2].plot(range(len(u)), u, 'D', label = 'Truth', color = 'red', alpha = 1)
        ax[2].plot(range(len(u)), x, 'o', label = 'IEKF-SL', color = 'blue', alpha = 1)
        ax[2].errorbar(range(len(u)), x, yerr = (x-x_l, x_u-x), fmt='.', color='blue', ecolor='skyblue', capsize=5, capthick = 1.25, label='IEKF-SL-CR')
        #ax[2].set_title('Recovery based on 7th L05-IEKF-SL', fontsize = 16)
        ax[2].set_title('Recovery based on 7th L1-IEKF-SL', fontsize = 16)
